...
def augment(
    ...
    try:
        payload = policy.load_json(evidence_path, "PiP performance evidence")
    except (OSError, policy.QualificationPolicyError) as error:
        raise PerformanceTraceError(
            f"cannot read PiP performance evidence: {error}"
        ) from error
    ...
    if scenario not in {
        ...
        raise PerformanceTraceError(
            "performance traces belong only to a PiP performance row"
        )
    ...
    try:
        policy.validate_host_augmented_artifacts(
            payload, scenario, evidence_path.parent, evidence_path.stem
        )
    except policy.QualificationPolicyError as error:
        shutil.rmtree(artifact_directory, ignore_errors=True)
        raise PerformanceTraceError(
            f"retained performance traces failed semantic validation: {error}"
        ) from error
