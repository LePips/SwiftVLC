...
Physical validation is mandatory before a release is represented as stable.
The human interaction is limited to connecting, unlocking, and trusting the
device; the runner performs the in-app work and generates the report.

1. Prepare one immutable release candidate:

   ```bash
   ./scripts/release.sh X.Y.Z --prepare /absolute/path/to/candidate
   ```

2. Build and run the validator from the exact candidate tag or checkout. Its
   `Package.swift` must pin the immutable candidate artifact; the launcher
   verifies that URL and checksum before building. For the normal one-command
   path:
   ...
   Repeat this command for every hardware/OS row in `matrix.json`. Omit
   `--only`: a focused diagnostic is useful while developing, but it is not a
   complete release run. Expect roughly 8–10 hours for the current-iPhone row.
   The device can be left unattended after the run starts, but it must remain
   powered, unlocked, trusted, and connected.

3. Keep each run's adjacent raw directory. The shareable ZIP is the handoff for
   issue reporters; release assembly consumes the raw `report.json` and its
   relative evidence files. Combine all eligible reports using the candidate
   metadata generated by the device runner:
   ...
   python3 scripts/qualification/assemble-record.py \
     ...
     --candidate-metadata /absolute/path/to/candidate-metadata.json \
     --matrix scripts/qualification/matrix.json \
     --report /absolute/path/to/iphone-minimum/report.json \
     --report /absolute/path/to/iphone-current/report.json \
     --report /absolute/path/to/ipad-current/report.json \
     --output scripts/qualification/X.Y.Z.json
   ```

4. Run the same fail-closed check used by stable publication:

   ```bash
   ./scripts/check-qualification.sh \
     X.Y.Z /absolute/path/to/candidate/libvlc.xcframework
   ```

   Do not continue if it reports a missing, failed, exploratory, or stale row.
   A successful test count alone is not the gate; the candidate-bound matrix
   record must pass this command.

5. Publish the already-qualified bytes without rebuilding them:

   ```bash
   ./scripts/release.sh X.Y.Z --candidate /absolute/path/to/candidate
   ```

Prereleases may be used to distribute a candidate and gather missing device
reports, but their notes must identify uncovered rows and they must not be
described as stable. Simulator results never substitute for physical evidence.
